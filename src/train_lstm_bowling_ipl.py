...
df = df.dropna(subset=["bowler", "venue", "bat_team", "wickets", "recent_form_avg_wkts", "date"])
...
for player in df["player"].unique():
    ...
    for i in range(len(group) - 5):
        seq = group.iloc[i:i+5][["recent_form_avg_wkts", "venue_enc", "oppo_enc"]].values
...
torch.save(model.state_dict(), "models/lstm_bowling_ipl.pt")
print(" Model saved to models/lstm_bowling_ipl.pt")
