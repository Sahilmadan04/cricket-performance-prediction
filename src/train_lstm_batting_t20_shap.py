...
df = df.dropna(subset=["player", "venue", "opposition", "runs", "recent_form_avg_runs", "date"])
...
input_seq = last_5[["recent_form_avg_runs", "venue_enc", "oppo_enc"]].values.astype(np.float32)

# -----------------------------
# Step 4: Load trained model
# -----------------------------
model = LSTMPredictor(input_dim=3)
model.load_state_dict(torch.load("models/lstm_batting_t20.pt"))
model.eval()
